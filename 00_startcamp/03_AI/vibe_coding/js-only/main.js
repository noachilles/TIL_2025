...
function updateGrid() {
  ...
  reader.onload = function(e) {
    ...
    if (emptyCount === 0) {
      ...
      drawFinalSeats(arrangeSeats(names, row, col, []));
      return;
    }
  ...
  const timer = setInterval(() => {
    ...
    if (count > 0) {
      ...
      startFirework(() => {
        ...
        drawFinalSeats(arrangeSeats(names, row, col, emptyPositions));
      });
    }
  }, 900);
}
...
// JS-only 자리배치 알고리즘
function arrangeSeats(names, num_rows, num_cols, empty_positions) {
  const total_seats = num_rows * num_cols;
  let seats = Array.from({length: num_rows}, () => Array(num_cols).fill(""));
  // 빈자리 좌표에 "(빈 자리)"를 먼저 채움
  for (const [r, c] of empty_positions) {
    seats[r][c] = "(빈 자리)";
  }
  // 남은 칸에 학생 무작위 배치
  let students = names.slice();
  ...
  for (let r = 0; r < num_rows; r++) {
    for (let c = 0; c < num_cols; c++) {
      if (seats[r][c] === "" && idx < students.length) {
